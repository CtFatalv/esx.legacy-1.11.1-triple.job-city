...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, job2, job3, loadout, name, coords, metadata)
    ...
    ---@param onDuty? boolean
    ---@return nil
    function self.setJob3(newJob3, grade3, onDuty)
        grade3 = tostring(grade3)
        local lastJob3 = self.job3

        if not ESX.DoesJobExist(newJob3, grade3) then
            return print(("[ESX] [^3WARNING^7] Ignoring invalid ^5.setJob3()^7 usage for ID: ^5%s^7, job3: ^5%s^7"):format(self.source, newJob3))
        end

        if newJob3 == "unemployed3" then
            onDuty = false
        end
